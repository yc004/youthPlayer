...
class Config:
    ...
    # 证书校验配置
    # 证书内容存储在数据库 SystemSetting(key='license_certificate') 中
    # 以下为运行时缓存
    LICENSE_CHECK_ENABLED = os.environ.get("YP_LICENSE_CHECK_ENABLED", "1") == "1"
    # 证书过期前 N 天开始警告
    LICENSE_EXPIRE_WARNING_DAYS = int(os.environ.get("YP_LICENSE_EXPIRE_WARNING_DAYS", 30))
